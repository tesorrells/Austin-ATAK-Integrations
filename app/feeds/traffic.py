...
class TrafficFeedPoller:
    ...
    async def _process_incident(self, incident: Dict[str, Any]) -> None:
        ...
        cot_sent = False
        if cot_sender.is_running:
            logger.info(f"Attempting to send CoT for traffic incident: {incident.get('traffic_report_id', 'unknown')}")
            cot_sent = await cot_sender.send_cot(cot_xml)
            if cot_sent:
                logger.info(f"Successfully sent CoT for traffic incident: {incident.get('traffic_report_id', 'unknown')}")
            else:
                logger.error(f"Failed to send CoT for traffic incident: {incident.get('traffic_report_id', 'unknown')}")
        else:
